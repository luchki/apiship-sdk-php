...
class Calculator extends AbstractApi
{
    ...
    public function calculate(CalculatorRequest $request, $headers = [])
    {
        ...
        return ($this->getResponseMaker())($resultJson);
    }

    public function calculateAsync(CalculatorRequest $request, $headers = []): PromiseInterface
    {
        $promise = $this->adapter->postAsync('calculator', $headers, $request->asJson());

        $response_maker = $this->getResponseMaker();

        return $promise->then(static function (ResponseInterface $response) use ($response_maker) {
            return $response_maker((string)$response->getBody());
        });
    }

    private function getResponseMaker(): callable
    {
        return static function (string $json): CalculatorResponse {
