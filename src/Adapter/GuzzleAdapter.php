...
class GuzzleAdapter extends AbstractAdapter implements AdapterInterface
{
    ...
    )
    {
        if (is_string($login) && $login && is_string($password) && $password) {
            ...
            throw new InvalidArgumentException(
                'Property "' . get_class($this) . '::login" and "' . get_class(
                    $this
                ) . '::password" might be a non empty string.'
            );
        }
    ...
    public function getLatestResponseHeaders()
    {
        ...
        $xTracingId = (string)isset($this->response->getHeader('x-tracing-id')[0]) ? $this->response->getHeader(
            'x-tracing-id'
        )[0] : '';
